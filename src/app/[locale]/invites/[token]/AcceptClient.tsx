...
export default function AcceptClient({ token }: { token: string }) {
  ...
  const onAccept = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/invites/token/${encodeURIComponent(token)}/accept`, {
        method: "POST",
      });
  ...
  if (!me) {
    const signInHref = `/users/sign_in?redirect=${encodeURIComponent(`/invites/${token}`)}`;
    const signUpHref = `/users/sign_up?redirect=${encodeURIComponent(`/invites/${token}`)}`;
    return (
      <div className={styles.card}>
        <div className={styles.brand}>sunday</div>
        <h1 className={styles.title}>
          Join {invite.workspaceTitle || "this workspace"}
        </h1>
        <p className={styles.lede}>
          {inviterName(invite)} invited you{invite.email ? ` at ${invite.email}` : ""}. Sign in or
  ...
  return (
    <div className={styles.card}>
      ...
      <p className={styles.lede}>
        {inviterName(invite)} invited you{invite.email ? ` at ${invite.email}` : ""}. You're signed
